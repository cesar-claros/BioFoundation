"""Plot ROC or precision-recall curves per model variant from the dumped eval scores.

Reads the per-cell score npz files written by ``eval_subject_level.py`` (``+dump_dir``),
i.e. ``sweep_foundation_models.py --dump_only --dump_dir <dir>``, and the HYDRA baseline dumped
by ``code/src/utils/trainer.py`` in the same schema, named

    w<ws>_s<seed>_<variant>_<mode>_<split>.npz

each holding the per-window (``win_prob``/``win_y``/``win_subject``) and per-subject
(``subj_prob``/``subj_y``) scores. For each window length it overlays the variants' mean curve
across seeds (with a +/-1 std band, computed by interpolating each seed's curve onto a common
grid) and reports the mean summary metric +/- std in the legend:

    --curve roc : ROC curve (x=false positive rate, y=true positive rate), summary = AUROC.
    --curve pr  : precision-recall curve (x=recall, y=precision), summary = average precision (AP).
...
# code/src/utils/trainer.py in the same npz schema), so all overlay in one panel.
...
GRID = np.linspace(0.0, 1.0, 201)  # common x grid: FPR (ROC) or recall (PR) for vertical averaging
# Per-curve axis labels, summary-metric name, and legend corner (ROC curves hug the top-left, PR
# curves the top-right, so place the legend where the curves are not).
CURVE_CFG = {
    "roc": {"metric": "AUC", "xlabel": "false positive rate (1 - specificity)",
            "ylabel": "true positive rate (sensitivity)", "legend_loc": "lower right", "title": "ROC"},
    "pr": {"metric": "AP", "xlabel": "recall (sensitivity)", "ylabel": "precision",
           "legend_loc": "lower left", "title": "precision-recall"},
}
...
def _curve(npz_path: Path, level: str, curve: str):
    """(y-on-GRID, summary_metric, positive_prevalence) for one dump, or None if single-class.

    For ROC the grid is FPR and y is the interpolated TPR (summary = AUROC); for PR the grid is
    recall and y is the interpolated precision (summary = average precision). The prevalence
    (fraction of positives) is returned so the PR panel can draw the no-skill baseline.
    """
    ...
    if curve == "roc":
        ...
        y_grid = np.interp(GRID, fpr, tpr)
        y_grid[0] = 0.0
        return y_grid, float(roc_auc_score(y, s)), prevalence
    # PR: precision_recall_curve returns recall DESCENDING (threshold ascending); sort ascending
    # so np.interp sees an increasing x-grid, then interpolate precision onto the common recall grid.
    prec, rec, _ = precision_recall_curve(y, s)
    order = np.argsort(rec)
    y_grid = np.interp(GRID, rec[order], prec[order])
...
def _panel(ax, recs_w, level, title, variants, curve):
    ...
    for variant in variants:
        ...
        for _ws, _seed, var, path in recs_w:
            if var != variant:
                continue
            r = _curve(path, level, curve)
            if r is not None:
                curves.append(r[0])
                scores.append(r[1])
                prevalences.append(r[2])
        if not curves:
            continue
...
def main() -> None:
    ...
    p.add_argument("--curve", default="roc", choices=["roc", "pr"],
                   help="roc = ROC curve (AUROC); pr = precision-recall curve (average precision). Default roc.")
    p.add_argument("--level", default="subject", choices=["subject", "window"],
                   help="Curve on subject-aggregated scores (mean-prob per subject) or per window.")
    ...
    fig.suptitle(f"LuMamba/HYDRA {cfg['title']} by variant "
                 f"({args.level}-level, {args.split}; mean ±1 std over seeds)", fontsize=12)
    fig.supxlabel(cfg["xlabel"])
    fig.supylabel(cfg["ylabel"])
    ...
    plt.close(fig)
    print(f"wrote {out}  ({args.curve}, {len(windows)} window panel(s), variants={plot_variants})")
